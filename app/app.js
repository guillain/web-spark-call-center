...
document.getElementById('icon-phone').addEventListener('click', () => {
  if (activeCall) {
    activeCall.hangup();
  }
});
...
document.getElementById('icon-audio').addEventListener('click', () => {
    if (activeCall) {
        if (activeCall.startSendingAudio()) {
            activeCall.stopSendingAudio();
        } else if (activeCall.stopSendingAudio()) {
...
document.getElementById('icon-video').addEventListener('click', () => {
    if (activeCall) {
        if (activeCall.startSendingVideo()) {
            activeCall.stopSendingVideo();
        }
        if (activeCall.stopSendingVideo()) {
            activeCall.startSendingVideo();
        }
    }
});

document.getElementById('icon-screen').addEventListener('click', () => {
  if (activeCall) {
      if (activeCall.startScreenShare()) {
          activeCall.stopScreenShare();
      }
      if (activeCall.stopScreenShare()) {
          activeCall.startScreenShare();
      }
  }
});

document.getElementById('icon-soft').addEventListener('click', () => {
  if (activeCall) {
      if (activeCall.startApplicationShare()) {
          activeCall.stopApplicationShare();
      }
      if (activeCall.stopApplicationShare()) {
